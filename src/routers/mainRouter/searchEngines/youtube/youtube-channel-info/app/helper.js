...
class YoutubeGrabberHelper {
  ...
  async parseChannelVideoResponse(response, channelId, channelIdType) {
    ...
    const channelVideoData = second.map(
      (item) => item?.richItemRenderer?.content || item
    );
    ...
    if (continuationItem.length > 0) {
      continuation =
        continuationItem[0].continuationItemRenderer.continuationEndpoint
    ...
    const latestVideos = channelVideoData
      .filter((item) => {
        return typeof item.continuationItemRenderer === "undefined";
      })
      .map((item) => {
  ...
  parseVideo(obj, channelInfo) {
    ...
    if (
      typeof obj.gridVideoRenderer === "undefined" &&
      typeof obj.videoRenderer !== "undefined"
    ) {
    ...
    const statusRenderer =
      video.thumbnailOverlays[0].thumbnailOverlayTimeStatusRenderer;
    ...
    if (
      ...
      publishedText = "Live";
      viewCount = parseInt(
        video.viewCountText.runs[0].text.split(",").join("")
      );
      viewCountText =
        video.shortViewCountText.runs[0].text +
        video.shortViewCountText.runs[1].text;
    } else if (
      ...
      viewCount = parseInt(
        video.viewCountText.simpleText.split(" ")[0].split(",").join("")
      );
      ...
      if (
        typeof video.thumbnailOverlays[0].thumbnailOverlayTimeStatusRenderer !==
        "undefined"
      ) {
        durationText =
  ...
  parseCommunityPage(communityInfo, channelIdType) {
    ...
    let contentDataString = communityInfo.data.match(
      /ytInitialData.+?(?=;<\/script>)/
    )[0];
    let innertubeAPIkey = communityInfo.data.match(
      /innertubeApiKey.+?(?=innertubeApiVersion)/
    )[0];
    ...
    contentDataString = contentDataString.substring(
      16,
      contentDataString.length
    );

    // Parse the JSON data and get the relevent array with data
    let contentDataJSON = JSON.parse(contentDataString);
    contentDataJSON =
      contentDataJSON.contents.twoColumnBrowseResultsRenderer.tabs[3]
        .tabRenderer.content.sectionListRenderer.contents[0]
        .itemSectionRenderer;
    if (
      "continuationItemRenderer" in
      contentDataJSON.contents[contentDataJSON.contents.length - 1]
    ) {
      return {
  ...
  createCommunityPostArray(postArray) {
    ...
    postArray.forEach((post) => {
      ...
      const postData = {
        postText: "",
        postId:
          post.backstagePostThreadRenderer.post.backstagePostRenderer.postId,
        author:
          post.backstagePostThreadRenderer.post.backstagePostRenderer.authorText
            .runs[0].text,
        authorThumbnails:
        ...
        voteCount:
          post.backstagePostThreadRenderer.post.backstagePostRenderer.voteCount
            .simpleText,
        postContent: null,
        commentCount:
          "text" in
          post.backstagePostThreadRenderer.post.backstagePostRenderer
            .actionButtons.commentActionButtonsRenderer.replyButton
            .buttonRenderer
            ? post.backstagePostThreadRenderer.post.backstagePostRenderer
                .actionButtons.commentActionButtonsRenderer.replyButton
      ...
      if (
        "runs" in
        post.backstagePostThreadRenderer.post.backstagePostRenderer.contentText
      ) {
      ...
      if (
        ...
        if (
          ...
          const pollObject =
            post.backstagePostThreadRenderer.post.backstagePostRenderer
              .backstageAttachment.pollRenderer;
          postData.postContent = {
            type: "poll",
            content: {
              choices: pollObject.choices.map(
                (entry) => entry.text.runs[0].text
              ),
          ...
          const playlistRenderer =
            post.backstagePostThreadRenderer.post.backstagePostRenderer
              .backstageAttachment.playlistRenderer;
          postData.postContent = {
            type: "playlist",
            content: {
  ...
  parseSharedPost(post) {
    const postData = {
      ...
      author:
        post.backstagePostThreadRenderer.post.sharedPostRenderer.displayName
          .runs[0].text,
      authorThumbnails:
        post.backstagePostThreadRenderer.post.sharedPostRenderer.thumbnail
          .thumbnails,
      publishedText:
        post.backstagePostThreadRenderer.post.sharedPostRenderer
          .publishedTimeText.runs[0].text,
      voteCount:
        post.backstagePostThreadRenderer.post.sharedPostRenderer.originalPost
          .backstagePostRenderer.voteCount.simpleText,
      postContent: null,
      commentCount:
        "text" in
        post.backstagePostThreadRenderer.post.sharedPostRenderer.originalPost
          .backstagePostRenderer.actionButtons.commentActionButtonsRenderer
          .replyButton.buttonRenderer
          ? post.backstagePostThreadRenderer.post.sharedPostRenderer
              .originalPost.backstagePostRenderer.actionButtons
              .commentActionButtonsRenderer.replyButton.buttonRenderer.text
              .simpleText
          : "0",
    };
  ...
  async decideUrlRequestType(channelId, urlAppendix, channelIdType) {
    switch (channelIdType) {
      case 0:
        return this.performChannelPageRequestWithFallbacks(
          channelId,
          urlAppendix
        );
      case 1:
